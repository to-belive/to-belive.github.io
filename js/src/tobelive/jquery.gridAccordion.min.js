...
a.remove();for(a=0;a<r;a++)ca(a);b.settings.slideshow&&T();q.hover(function(){b.settings.slideshow&&b.settings.stopSlideshowOnHover&&G&&clearInterval(G)},function(){b.settings.closePanelOnMouseOut&&da();b.settings.slideshow&&b.settings.stopSlideshowOnHover&&T()})}function ca(a){var f=d('<div class="panel"></div>').appendTo(q);h.push(f);g[a].width=R;g[a].height=S;d("<img/>").load(function(){
    console.log(h[a]);
    console.log(d(this));
    h[a].css("background-image","url("+d(this).attr("src")+")");switch(g[a].properties.alignType){case "leftTop":h[a].css("background-position",
